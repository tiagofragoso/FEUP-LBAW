...
if (acquireTicketBtn !== null) {
    ...
    acquireTicketBtn.addEventListener('click', async () => {
        ...
        if (response.status === 200) {
            ...
            let price = acquireTicketBtn.closest('.modal-content').querySelector('.modal-body').innerText;
            ...
            $('#acquireTicketModal').on('hidden.bs.modal', function (e) {
                ...
                acquireTicketBtn.closest('.modal-content').querySelector('.modal-body').innerText = price;
              })
...
document.querySelectorAll('.full-ticket').forEach( ticket => {
    var qrcode = new QRCode(ticket.querySelector('.qrcode'), {
        text: ticket.dataset.code,
        width: 230,
        height: 230,
        colorDark : "#000000",
        colorLight : "#ffffff",
